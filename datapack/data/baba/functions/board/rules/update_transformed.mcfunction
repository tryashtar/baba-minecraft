execute as @e[type=armor_stand,tag=baba.object,tag=transformed] run data modify entity @s HandItems[0].tag.parsing set value {delete:0b,block_transforms:0b,transforms:[],writes:[],properties:[],has:[],make:[]}
...
execute as @e[type=armor_stand,tag=baba.object,tag=transformed] run function baba:board/rules/finalize
