...
execute if score inverted baba matches 0 as @e[type=armor_stand,tag=baba.object,tag=!self] if score @s sprite = check_sprite baba run scoreboard players set passed baba 0
execute if score inverted baba matches 1 as @e[type=armor_stand,tag=baba.object,tag=!self,tag=!not_all] unless score @s sprite = check_sprite baba run scoreboard players set passed baba 0
