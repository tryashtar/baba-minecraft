...
execute if score inverted baba matches 0 as @e[type=item_display,tag=baba.object,tag=!self,tag=!used] if score @s sprite = check_sprite baba run tag @s add found
execute if score inverted baba matches 1 as @e[type=item_display,tag=baba.object,tag=!self,tag=!used,tag=!not_all] unless score @s sprite = check_sprite baba run tag @s add found
