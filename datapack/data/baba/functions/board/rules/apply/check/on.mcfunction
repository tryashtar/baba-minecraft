...
execute if score inverted baba matches 0 as @e[type=armor_stand,tag=baba.object,tag=!self,distance=..0.1] if score @s sprite = check_sprite baba run scoreboard players set passed baba 1
execute if score inverted baba matches 1 as @e[type=armor_stand,tag=baba.object,tag=!self,tag=!not_all,distance=..0.1] unless score @s sprite = check_sprite baba run scoreboard players set passed baba 1
