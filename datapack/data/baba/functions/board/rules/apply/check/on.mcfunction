...
execute if score inverted baba matches 0 as @e[type=#baba:object,tag=baba.object,tag=!self,tag=!used,distance=..0.1] if score @s sprite = check_sprite baba run tag @s add found
execute if score inverted baba matches 1 as @e[type=#baba:object,tag=baba.object,tag=!self,tag=!used,tag=!not_all,distance=..0.1] unless score @s sprite = check_sprite baba run tag @s add found
