...
scoreboard players operation @e[type=marker,tag=baba.rule,tag=disabler] sprite -= @s sprite
scoreboard players operation @e[type=marker,tag=baba.rule,tag=disabler] life -= @s life
scoreboard players operation @e[type=marker,tag=baba.rule,tag=disabler] text -= @s text
...
# 'A verb X' is disabled by 'A verb not X'
execute if entity @s[tag=!subject_inverted] at @e[type=marker,tag=baba.rule,tag=disabler,tag=!changed,tag=!subject_inverted,tag=!subject_all,scores={life=0,text=0,sprite=0}] run function baba:board/rules/disabling/update_disabled
scoreboard players operation @e[type=marker,tag=baba.rule,tag=disabler] sprite += @s sprite
scoreboard players operation @e[type=marker,tag=baba.rule,tag=disabler] life += @s life
scoreboard players operation @e[type=marker,tag=baba.rule,tag=disabler] text += @s text
