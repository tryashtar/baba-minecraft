...
execute if entity @s[tag=!subject_inverted] as @e[type=item_display,tag=baba.object,tag=joined_group,tag=assign] at @s run function baba:board/rules/apply/check
execute if entity @s[tag=subject_inverted] as @e[type=item_display,tag=baba.object,tag=!joined_group,tag=!not_all,tag=assign] at @s run function baba:board/rules/apply/check
