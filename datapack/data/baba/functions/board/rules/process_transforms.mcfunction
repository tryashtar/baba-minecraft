...
execute if entity @e[type=marker,tag=baba.object,tag=transformed,limit=1] run function baba:board/rules/apply_rules_transformed
execute as @e[type=marker,tag=baba.object,tag=transformed,nbt={data:{sprite:"text"}}] run data modify entity @s data.properties append value {property:"push",inverted:0b}
