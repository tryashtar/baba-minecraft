...
tag @e[type=armor_stand,tag=baba.object,tag=pending_all_rules] add all_rules
scoreboard players add @e[type=armor_stand,tag=baba.object,tag=subject] text_using 1
scoreboard players operation @e[type=armor_stand,tag=baba.object,tag=last_verb] text_using += rule_count baba
...
data modify storage baba:main parsing.rules[-1][].effect.text_text set from entity @s HandItems[0].tag.text
...
execute unless entity @s[tag=part.noun] positioned ^ ^ ^1 as @e[type=armor_stand,tag=baba.object,tag=reparse,tag=!part.and,distance=..0.1] run function baba:board/rules/parse/new
