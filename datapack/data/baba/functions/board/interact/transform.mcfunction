data modify storage baba:main transform set from entity @s HandItems[0].tag.transforms[0]
...
execute if data storage baba:main transform{write:1b} run scoreboard players set spawn baba 30442
...
data remove entity @s HandItems[0].tag.transforms[0]
# only kill when all transformations are done, because killing removes scores
execute unless data entity @s HandItems[0].tag.transforms[0] run function baba:board/interact/remove
execute if data entity @s HandItems[0].tag.transforms[0] run function baba:board/interact/transform
