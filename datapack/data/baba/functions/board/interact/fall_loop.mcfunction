...
execute if score open baba matches 1 as @e[type=item_display,tag=baba.object,distance=..0.1,nbt={item:{tag:{properties:["shut"]}}}] if score @s float_level = float baba run tag @s add open
execute if score open baba matches 2 as @e[type=item_display,tag=baba.object,distance=..0.1,nbt={item:{tag:{properties:["open"]}}}] if score @s float_level = float baba run tag @s add open
...
execute if entity @e[type=item_display,tag=baba.object,tag=!falling,distance=..0.1,nbt={item:{tag:{properties:["weak"]}}},limit=1] as @a at @s run playsound baba:defeat master @s
execute as @e[type=item_display,tag=baba.object,tag=!falling,distance=..0.1,nbt={item:{tag:{properties:["weak"]}}}] run function baba:board/interact/destroy
