...
execute if entity @s[tag=!teleported,tag=!swap,tag=!falling] run tag @e[type=item_display,tag=baba.object,nbt={item:{tag:{properties:["swap"]}}},distance=..0.1] add swap
execute if entity @s[tag=!teleported,tag=!swap,tag=!falling,nbt={item:{tag:{properties:["swap"]}}}] run tag @e[type=item_display,tag=baba.object,nbt=!{item:{tag:{properties:["swap"]}}},distance=..0.1] add swap
