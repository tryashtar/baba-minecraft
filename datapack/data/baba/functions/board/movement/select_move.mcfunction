...
execute if entity @s[tag=!can_move] if entity @e[type=item_display,tag=baba.object,distance=..0.1,nbt={item:{components:{"minecraft:custom_data":{level_data:{}}}}},limit=1] run tag @s add can_move
