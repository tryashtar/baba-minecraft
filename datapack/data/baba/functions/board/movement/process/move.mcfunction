execute as @e[type=armor_stand,tag=baba.object,tag=!move_done,nbt={HandItems:[{tag:{properties:["move"]}}]},nbt=!{HandItems:[{tag:{properties:["sleep"]}}]},tag=!move_done] at @s run function baba:board/movement/attempt/move
