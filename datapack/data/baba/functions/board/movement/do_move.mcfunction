...
execute as @e[type=item_display,tag=baba.object,tag=destroy] at @s run function baba:board/interact/destroy
scoreboard players add @e[type=item_display,tag=baba.object,tag=can_move] walk 1
scoreboard players set @e[type=item_display,tag=baba.object,tag=can_move,scores={walk=4..}] walk 0
...
tag @e[type=item_display,tag=baba.object,tag=swapped] remove swapped
