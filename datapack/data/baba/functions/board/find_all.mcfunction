...
execute if score sprite baba matches 1.. as @e[type=armor_stand,tag=baba.object,tag=!found] if score @s sprite = sprite baba run tag @s add found
execute if score sprite baba matches 1.. as @e[type=armor_stand,tag=baba.object,tag=part.noun,tag=!special,tag=!found_text] if score @s text = sprite baba run tag @s add found_text
