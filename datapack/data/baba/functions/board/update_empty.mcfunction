...
execute at @e[type=marker,tag=baba.space] unless entity @e[type=#baba:object,tag=baba.object,distance=..0.1,limit=1] run summon marker ~ ~ ~ {Tags:["baba.object","not_all","empty","spawn","assign"]}
scoreboard players set @e[type=marker,tag=baba.object,tag=empty,tag=spawn] sprite 2925313
execute as @e[type=marker,tag=baba.object,tag=empty,tag=spawn] store result score @s facing run loot spawn ~ ~ ~ loot baba:random_4
