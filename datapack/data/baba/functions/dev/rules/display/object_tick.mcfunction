...
data modify storage baba:main debug_text set from entity @s item.components."minecraft:custom_data".text
scoreboard players set spawn baba 1
execute as @e[type=text_display,tag=rule_display,predicate=baba:same_text_id] run function baba:dev/rules/display/update
