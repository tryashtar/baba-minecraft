execute as @e[type=marker,tag=spawn_tile_editor] at @s align xyz if block ~ ~-1 ~ chiseled_bookshelf run function baba:dev/tile_editor/open
...
execute as @e[type=marker,tag=spawn_copier] at @s if block ~ ~-1 ~ chiseled_bookshelf run function baba:dev/copy_tile
