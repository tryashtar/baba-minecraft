...
data modify storage baba:main link set from entity @s item.components."minecraft:custom_data".level_data.link
...
execute at @e[type=marker,tag=baba.storage,limit=1] run data modify storage baba:main level_name set from block ~ ~ ~ RecordItem.components."minecraft:custom_data".metadata.name
