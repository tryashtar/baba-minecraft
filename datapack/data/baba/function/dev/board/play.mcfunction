$data modify block ~ ~ ~ front_text.messages[1] set value '{"text":"Stop","clickEvent":{"action":"run_command","value":"/execute rotated $(rot) run function baba:dev/board/stop {facing:\\"$(facing)\\",rot:\\"$(rot)\\",start:\\"$(start)\\",storage:\\"$(storage)\\",meta:\\"$(meta)\\",name:\\"$(name)\\",load_meta:\\"$(load_meta)\\",load_name:\\"$(load_name)\\"}"}}'
...
$execute positioned $(start) align xyz positioned ^0.5 ^0.5 ^ run function baba:editor/load
