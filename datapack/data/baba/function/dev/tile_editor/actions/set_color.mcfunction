...
execute at @e[type=item_display,tag=baba.object,tag=tile_editor_preview,sort=nearest,limit=1] store result block ~ ~-1 ~ Items[0].components."minecraft:custom_data".baba.extra.scores.color int 1 run scoreboard players get pick_color baba
execute at @e[type=item_display,tag=baba.object,tag=tile_editor_preview,sort=nearest,limit=1] store result block ~ ~-1 ~ bees[0].entity_data.extra.scores.color int 1 run scoreboard players get pick_color baba
