...
execute at @s run tag @e[type=#baba:object,tag=baba.object,tag=assign_move,distance=..1.5] add assign
execute at @s[tag=is_text] run tag @e[type=item_display,tag=baba.object,tag=is_text,distance=..1.1] add reparse
tp @s ~ ~ ~
