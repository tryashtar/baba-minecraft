...
scoreboard players remove @e[type=marker,tag=baba.space,tag=active] repeats 1
execute as @e[type=marker,tag=baba.space,tag=active,scores={repeats=0}] at @s run function baba:board/history/pop
...
execute if score repeats baba matches 1 run kill @e[type=marker,tag=baba.rule,tag=active]
