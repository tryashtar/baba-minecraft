...
def generate_update_function(source, rsources):
  ...
  for overlay in source.overlays.values():
    ...
    for spr in overlay.sprites:
      ...
      final += f'if entity @s[{selector}] run summon item_display ~ ~ ~ {{teleport_duration:3,width:1f,height:0.1f,item_display:"fixed",item:{{id:"potion",count:1,components:{{item_model:"{rsources[spr].model_component}",potion_contents:{{custom_color:{int(spr.properties[source.properties["color"]][1:],16)}}}}}}},Tags:["baba","baba.overlay"]}}'
      lines.append(final)
